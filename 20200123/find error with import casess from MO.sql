...
--параметры которые подаются в процедуру, выполняющую технологический контроль
	...
	begin	
		...
		--добавил условие что если в реестре все случаи в ошибках, то у них должен
		if exists(select 1 FROM t_RefCasePatientDefine t1 
				  WHERE t1.rf_idFiles=@idFile 
                  AND NOT EXISTS(SELECT 1 FROM dbo.t_ErrorProcessControl e WHERE e.rf_idCase=t1.rf_idCase
								 UNION ALL
                                 SELECT 1 FROM t_CasePatientDefineIteration t2 WHERE t2.rf_idRefCaseIteration=t1.id
								)
...
--закончить данную обработку
		...
		IF EXISTS(SELECT 1 FROM dbo.vw_getFileBack WHERE idFileBack=@idFileBack AND Period<201811)
		BEGIN
			exec usp_RegisterSP_TK @idFileBack
		END
		ELSE 
