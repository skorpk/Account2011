...
FROM t_FileBack fb inner join t_RegisterCaseBack ab on
			...
			INNER JOIN dbo.t_Case c ON 
					...
					AND c.DateEnd>=@dateReg
					AND c.DateEnd<=GETDATE()			
WHERE fb.DateCreate>@dateReg AND fb.DateCreate<GETDATE() AND NOT EXISTS(SELECT 1 FROM dbo.vw_ErrorProcessControl e WHERE e.rf_idRecordCase=c.rf_idRecordCase)
--очищаю таблицу
