...
/****** Object:  View [dbo].[vw_sprMU]    Script Date: 13.03.2019 10:36:47 ******/
SET ANSI_NULLS ON
GO
...
SELECT DISTINCT CAST(MUGroupCode AS varchar(2)) + '.' + CAST(MUUnGroupCode AS varchar(2)) + '.' + CAST(MUCode AS varchar(3)) AS MU, AdultUET, ChildUET, 
                unitCode, unitName,MUGroupCode,MUUnGroupCode,MUCode,beginDate,endDate,calculationType,IsNextVisit,NomenclMUCode
FROM         (
				select m.MUId,m.MUGroupCode,m.MUUnGroupCode,m.MUCode,m.AdultUET,m.ChildUET
						...
						,cc.MUUnGroupCodeP,cc.MUCodeP,m.MUName,m.IsCompletedCase, m.IsNextVisit
						,unit.calculationType, m.NomenclMUCode
