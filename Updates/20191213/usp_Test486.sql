USE [RegisterCases]
GO
if(OBJECT_ID('usp_Test486',N'P')) is not null
	drop PROCEDURE dbo.usp_Test486
go
CREATE PROC [dbo].[usp_Test486]
		@idFile INT,
		@month tinyint,
		@year smallint,
		@codeLPU char(6)		
...
from t_File f INNER JOIN t_RegistersCase a ON
		...
		c.id=ad.rf_idCase   				
         ...
         c.id=u.rf_idCase
where a.rf_idFiles=@idFile AND f.TypeFile='H' AND c.age>17 AND EXISTS(SELECT * FROM dbo.t_ONK_SL d WHERE d.rf_idCase=c.id AND d.DS1_T <3)
		AND ad.rf_idAddCretiria LIKE 'sh%' AND u.rf_idN013=2 AND EXISTS(SELECT 1 FROM dbo.t_AdditionalCriterion aa WHERE u.rf_idCase=c.id AND aa.rf_idAddCretiria NOT LIKE 'sh%')
UNION ALL
SELECT DISTINCT c.id,486
from t_File f INNER JOIN t_RegistersCase a ON
		f.id=a.rf_idFiles
		...
		d.DS2=dd.DiagnosisCode  				 				
         ...
         c.id=u.rf_idCase
where a.rf_idFiles=@idFile AND f.TypeFile='H' AND c.age>17 AND EXISTS(SELECT * FROM dbo.t_ONK_SL d WHERE d.rf_idCase=c.id AND d.DS1_T <3)
		AND u.rf_idN013=2 AND NOT NOT EXISTS(SELECT 1 FROM dbo.t_AdditionalCriterion aa WHERE u.rf_idCase=c.id AND aa.rf_idAddCretiria LIKE 'sh%')
...
GO
GRANT EXECUTE ON usp_Test486 TO db_RegisterCase
go
