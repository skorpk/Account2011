...
/****** Object:  StoredProcedure [dbo].[usp_PlanOrdersReport]    Script Date: 03/12/2012 16:36:46 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[usp_PlanOrdersReport]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[usp_PlanOrdersReport]
GO
...
create procedure [dbo].[usp_PlanOrdersReport]
@idFile int=null,
				@idFileBack int
as
...
(
		...
		,QuarterName as (case when QuarterID=1 then 'первый квартал'
								...
								when QuarterID=1 then 'третий квартал' else 'четвертый квартал' end)
)
