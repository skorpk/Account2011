...
alter procedure [dbo].[usp_GetMeduslugiFromRegisterCaseDB]
		@account varchar(15)
		--,@rf_idF003 char(6)
		--,@month tinyint
		--,@year smallint
AS
		...
		m.MUCode,m.Quantity,m.Price,m.TotalPrice,m.rf_idV004,m.Comments,m.rf_idDoctor
		...
		------------------------------------------------------------------------------------------------------------
		--добавил медуслуги в св€зи с тем что ввели хирургический койко-день
		insert #meduslugi
		select c.GUID_Case,m.id,m.GUID_MU,m.rf_idMO,m.rf_idV002,m.MUSurgery,m.IsChildTariff,m.DateHelpBegin,m.DateHelpEnd,m.DiagnosisCode,
		m.MUCode,m.Quantity,m.Price,m.TotalPrice,m.rf_idV004,m.Comments,m.rf_idDoctor
		from #case1 c inner join RegisterCases.dbo.t_Meduslugi m on
		...
		from #case1 c inner join RegisterCases.dbo.t_Meduslugi m on
								...
								c.id=mes.rf_idCase
		------------------------------------------------------------------------------------------------------------						
		insert #meduslugi ----добавление случаев по дневному стационару, с 01.04.2013 дневной стационар идет как «—
		select c.GUID_Case,m.id,m.GUID_MU,m.rf_idMO,m.rf_idV002,m.MUSurgery,m.IsChildTariff,m.DateHelpBegin,m.DateHelpEnd,m.DiagnosisCode,
				m.MUCode,m.Quantity,m.Price,m.TotalPrice,m.rf_idV004,m.Comments,m.rf_idDoctor
		from #case1 c inner join RegisterCases.dbo.t_Meduslugi m on
								c.id=m.rf_idCase					
...
GRANT EXECUTE ON  [dbo].[usp_GetMeduslugiFromRegisterCaseDB] TO [db_AccountOMS]
GO