USE [AccountOMS]
GO
IF OBJECT_ID('usp_TestAccountDataLPUFileH',N'P') IS NOT NULL
	DROP PROCEDURE usp_TestAccountDataLPUFileH

GO
CREATE proc [dbo].[usp_TestAccountDataLPUFileH]
			@doc xml,
			@patient xml,
			@file varbinary(max),
			@fileName varchar(26),
...
insert #t3
SELECT N_ZAP,PR_NOV,ID_PAC,VPOLIS,SPOLIS,NPOLIS,ENP,ST_OKATO,SMO,SMO_OK,SMO_NAM,NOVOR,MO_PR,VNOV_D--new
...
insert #t5(N_ZAP ,ID_PAC ,IDCASE ,ID_C ,USL_OK ,VIDPOM 
			...
			,VID_HMP
			,METOD_HMP 
			,NPR_MO ,EXTR ,LPU ,PROFIL ,DET, TAL_D ,TAL_P ,NHISTORY ,
			P_PER ,DATE_1 ,DATE_2 ,DS0 ,DS1 ,CODE_MES1 ,RSLT ,ISHOD 
			,PRVS,OS_SLUCH ,IDSP ,ED_COL ,TARIF ,SUMV ,COMENTSL ,F_SP,IDDOCT,IT_SL )
SELECT N_ZAP,ID_PAC,IDCASE,ID_C,USL_OK,VIDPOM,
		...
		NPR_MO,EXTR,LPU,PROFIL,DET,TAL_D,TAL_P, NHISTORY, P_PER,replace(DATE_1,'-',''),replace(DATE_2,'-',''),DS0,DS1,CODE_MES1,RSLT,ISHOD,--new
		PRVS,OS_SLUCH,IDSP,ED_COL,TARIF,SUMV,COMENTSL,F_SP,IDDOKT,IT_SL
...
begin
	...
	from #case t inner join #t5 t1 on
			...
			END
			--проверяем талоны
			IF EXISTS(SELECT * FROM #tTalon b WHERE NOT EXISTS(SELECT * FROM #t5 WHERE b.GUID_Case=ID_C AND Tal_D=b.Tal_D AND Tal_P=b.Tal_P))
			BEGIN 			
				insert @et values(588,15)
			END          
end
