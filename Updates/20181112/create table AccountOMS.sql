...
(
	id ASC
)WITH (PAD_INDEX = OFF, STATISTICS_NORECOMPUTE = OFF, IGNORE_DUP_KEY = OFF, ALLOW_ROW_LOCKS = ON, ALLOW_PAGE_LOCKS = ON) ON AccountMU
) ON AccountMU
GO 
ALTER TABLE dbo.t_CompletedCase  WITH CHECK ADD  CONSTRAINT FK_CompCases_Patient FOREIGN KEY(rf_idRecordCasePatient)
