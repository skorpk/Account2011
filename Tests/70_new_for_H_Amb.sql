...
select @idFile=id from vw_getIdFileNumber where CodeM='141023' and NumberRegister=52 and ReportYear=2021
...
BEGIN 
		...
		----проверка на дулированность случая по пациенту в РС по стационару и дневному стационару
			...
			SELECT distinct rf_idCase,71,cteDouble.NumberCase FROM cteDouble WHERE idRow>1

			SELECT * FROM #tabAmb s1 WHERE s1.NumberCase IN(2,3,4)
	------------------------------------------------------70-----------------------------------	
