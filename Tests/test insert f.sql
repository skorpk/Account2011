...
FROM	OPENROWSET(BULK 'd:\Test\FRM391002T34_200100021.xml',SINGLE_BLOB) HRM (ZL_LIST)

SELECT	@patient=LRM.PERS_LIST				
FROM	OPENROWSET(BULK 'd:\Test\LRM391002T34_200100021.xml',SINGLE_BLOB) LRM (PERS_LIST)
...
insert #t5( N_ZAP, ID_PAC ,IDCASE ,ID_C ,DISP ,USL_OK ,VIDPOM ,FOR_POM ,LPU ,PROFIL ,VBR ,NHISTORY ,P_OTK ,DATE_1 ,DATE_2 ,DS1 ,
			DS1_PR ,PR_D_N ,CODE_MES1 ,RSLT ,ISHOD ,PRVS ,IDSP ,ED_COL ,TARIF ,SUMV ,COMENTSL, LPU_1, DS_ONK,P_CEL,SL_ID,Date_Z_1,Date_Z_2)
SELECT     N_ZAP,ID_PAC, IDCASE, ID_C, DISP, USL_OK, VIDPOM,FOR_POM, LPU,PROFIL, VBR, NHISTORY,P_OTK,replace(DATE_1,'-',''),replace(DATE_2,'-',''),DS1
		   ,DS1_PR, PR_D_N, CODE_MES1,RSLT,ISHOD,PRVS, IDSP,ED_COL,TARIF,SUMV,COMENTSL,LPU_1, DS_ONK, P_CEL, SL_ID,replace(DATE_Z_1,'-',''),replace(DATE_Z_2,'-','')
...
------Insert into RegisterCase's tables------------------------------
		...
		--		t1.N_ZAP=t2.N_ZAP and
		--		t1.ID_PAC=t2.ID_PAC
		--WHERE date_z_1 IS NULL



		INSERT dbo.t_CompletedCase( rf_idRecordCase ,idRecordCase ,GUID_ZSL ,DateBegin ,DateEnd ,AmountPayment)
		SELECT DISTINCT t2.id,t1.IDCASE,t1.ID_C,Date_Z_1,Date_Z_2,SUMV
		from #t5 t1 inner join @tempID t2 on
				t1.N_ZAP=t2.N_ZAP and
				t1.ID_PAC=t2.ID_PAC
		...
		select t2.id,t1.DOCTYPE,t1.DOCSER,t1.DOCNUM,t1.SNILS,t1.OKATOG,t1.OKATOP,t1.COMENTP,DOCDATE
			,CASE WHEN LEN(DOCORG)<1 THEN NULL ELSE t1.DOCORG end
		...
		where (t1.DOCTYPE is not null) or (t1.DOCSER is not null) or (t1.DOCNUM is not null) OR (t1.SNILS IS NOT null) OR (t1.OKATOG IS NOT NULL) OR (t1.OKATOP IS NOT NULL)
				OR (t1.DOCDATE IS NOT null) OR (t1.DOCORG IS NOT null)
		group by t2.id,t1.DOCTYPE,t1.DOCSER,t1.DOCNUM,t1.SNILS,t1.OKATOG,t1.OKATOP,t1.COMENTP,DOCDATE,CASE WHEN LEN(DOCORG)<1 THEN NULL ELSE t1.DOCORG end
