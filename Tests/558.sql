...
select @idFile=id from vw_getIdFileNumber where CodeM='103001' and NumberRegister=564 and ReportYear=2019
...
select distinct c.id,558,mes,rf_idV010,r.ID_Patient
from t_RegistersCase a inner join t_RecordCase r on
			a.id=r.rf_idRegistersCase
			and a.rf_idFiles=@idFile 
						inner join t_PatientSMO s on
...
SELECT * FROM vw_sprMU_CSG_Payment WHERE MUCode='ds19.023'
