...
(
	...
	FileXML varbinary(MAX) FILESTREAM
)
go
...
(
	...
	CONSTRAINT PK_RegisterCases_idFiles_idRegisterCases PRIMARY KEY CLUSTERED (rf_idFiles, id)
) 
go
...
(
	rf_idFiles int not null CONSTRAINT FK_RecordCase_Files FOREIGN KEY(rf_idFiles) REFERENCES dbo.t_File(id),
	id smallint not null,	
	IsNew bit,
	CONSTRAINT PK_RecordCase_idFiles_idRecordCase PRIMARY KEY CLUSTERED (rf_idFiles, id)
)
go
...
(
	...
	GUID_Case uniqueidentifier not null PRIMARY KEY,
	rf_idV006 tinyint,
	rf_idV008 smallint,
	rf_idDirectMO char(6) null,
	HopitalisationView tinyint null,
	rf_idMO char(6) not null,
	...
	CONSTRAINT FK_Cases_Files FOREIGN KEY(rf_idFiles,rf_idRecordCase) REFERENCES dbo.t_RecordCase(rf_idFiles,id)
)
go
...
(
	...
	CONSTRAINT FK_Diagnosis_Cases FOREIGN KEY(GUID_Case) REFERENCES dbo.t_Case(GUID_Case)
)
go
...
(
	...
	CONSTRAINT FK_MES_Cases FOREIGN KEY(GUID_Case) REFERENCES dbo.t_Case(GUID_Case)
)
go
if(OBJECT_ID('t_ReasonPaymentCanseled',N'U')) is not null
	drop table dbo.t_ReasonPaymentCanseled
go
create table dbo.t_ReasonPaymentCanseled
(
	GUID_Case uniqueidentifier not null,
	rf_idPaymentAccountCanseled tinyint not null,
	CONSTRAINT FK_ReasonPaymentCanseled_Cases FOREIGN KEY(GUID_Case) REFERENCES dbo.t_Case(GUID_Case)
)
go
...
(
	...
	CONSTRAINT FK_FinancialSanctions_Cases FOREIGN KEY(GUID_Case) REFERENCES dbo.t_Case(GUID_Case)
)
go
...
(
	...
	rf_idDoctor char(16) null,
	Comments nvarchar(250) null,
	CONSTRAINT FK_Meduslugi_Cases FOREIGN KEY(GUID_Case) REFERENCES dbo.t_Case(GUID_Case)
)
go
if(OBJECT_ID('t_Patient',N'U')) is not null
...
(
	...
	CONSTRAINT PK_Patient PRIMARY KEY CLUSTERED(rf_idFiles,rf_idRecordCase,GUID_Patient),
	CONSTRAINT FK_Patient_RecordCase FOREIGN KEY(rf_idFiles,rf_idRecordCase) REFERENCES dbo.t_RecordCase(rf_idFiles,id)
)
go
...
(
	...
	CONSTRAINT FK_PatientSMO_Patient FOREIGN KEY(rf_idFiles,rf_idRecordCase,GUID_Patient) REFERENCES dbo.t_Patient(rf_idFiles,rf_idRecordCase,GUID_Patient)
)
go
