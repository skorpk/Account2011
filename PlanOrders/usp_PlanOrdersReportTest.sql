use RegisterCases
go
if OBJECT_ID('usp_PlanOrdersReportTest',N'P') is not null
drop proc usp_PlanOrdersReportTest
go
create procedure usp_PlanOrdersReportTest
@idFile int=null,
				@idFileBack int
as
...
insert @p
SELECT distinct cb.id--p.rf_idRecordCaseBack
...
from (
		...
		from t_FileBack f inner join t_RegisterCaseBack r on
				f.id=r.rf_idFilesBack		
				AND f.DateCreate>=@dateStart AND f.DateCreate<=@dateCreate
						  inner join t_RecordCaseBack cb on
				cb.rf_idRegisterCaseBack=r.id and
				r.ReportMonth>=@monthMin and r.ReportMonth<=@monthMax and
				r.ReportYear=@year
