...
FROM dbo.vw_getFileBack WHERE ReportYear=2016 AND CodeM='341001' AND NumberRegister=22 AND PropertyNumberRegister=1
...
insert @tPlan(tfomsCode,unitCode) select @codeLPU, unitCode from OMS_NSI_test.dbo.tPlanUnit
...
from @tPlan p inner join (
						...
						from OMS_NSI_test.dbo.tPlanYear py inner join OMS_NSI_test.dbo.tMO mo on
									py.rf_MOId=mo.MOId and
									py.[year]=@year
										inner join OMS_NSI_test.dbo.tPlan pl on
									py.PlanYearId=pl.rf_PlanYearId and 
									pl.flag='A'
										inner join OMS_NSI_test.dbo.tPlanUnit pu on
									pl.rf_PlanUnitId=pu.PlanUnitId
										left join OMS_NSI_test.dbo.tPlanCorrection pc on
	...
	SELECT  r.NumberRegister,c.rf_idMO
		...
		from t_FileBack f inner join t_RegisterCaseBack r on
				...
				c.id=m.rf_idCase and c.rf_idMO=@codeLPU
						inner join dbo.vw_sprMU_test t1 on
				m.MUCode=t1.MU			
				and t1.unitCode is not null
						inner join @p p on
				cb.id=p.id	
		WHERE c.DateEnd>= t1.beginDate AND c.DateEnd<=t1.endDate AND NumberRegister=22 AND t1.unitCode=32
...
-- отбираем список ЗС по реестру	
		SELECT   c.id,r.NumberRegister,c.rf_idMO
				,t1.unitCode
				,m.Quantity
				,m.MES
		from t_FileBack f inner join t_RegisterCaseBack r on
				...
				c.id=m.rf_idCase and c.rf_idMO=@codeLPU
						inner join (SELECT MU,beginDate,endDate,unitCode,ChildUET,AdultUET FROM dbo.vw_sprMU_test 
									UNION ALL 
									SELECT CSGCode,beginDate,endDate,UnitCode,ChildUET, AdultUET FROM OMS_NSI_test.dbo.vw_CSGPlanUnit
									) t1 on
				m.MES=t1.MU																	
		WHERE c.DateEnd>= t1.beginDate AND c.DateEnd<=t1.endDate AND NumberRegister=22 AND t1.unitCode=32--добавил фильтр по дате действия единиц учета				
		ORDER BY r.NumberRegister,c.id				
