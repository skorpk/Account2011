...
from t_FileBack f inner join t_RegisterCaseBack r on
		...
		and t1.unitCode is not null
				inner join #p p on
		cb.id=p.id	
WHERE c.DateEnd>= t1.beginDate AND c.DateEnd<=t1.endDate AND t1.calculationType=1 AND unitCode IN (30,31,32)
...
from t_FileBack f inner join t_RegisterCaseBack r on
		...
		and t1.unitCode is not null
				inner join #p p on
		cb.id=p.id							
WHERE c.DateEnd>= t1.beginDate AND c.DateEnd<=t1.endDate AND unitCode IN (30,31,32)
...
from t_FileBack f inner join t_RegisterCaseBack r on
		...
		and t1.unitCode is not null
				inner join #p p on
		cb.id=p.id							
WHERE c.DateEnd>= t1.beginDate AND c.DateEnd<=t1.endDate AND unitCode=32
 UNION all
select  c.id
		,c.GUID_Case
		,m.MES
...
from t_FileBack f inner join t_RegisterCaseBack r on
		...
		and t1.unitCode is not null
				inner join #p p on
		cb.id=p.id							
WHERE c.DateEnd>= t1.beginDate AND c.DateEnd<=t1.endDate AND unitCode =1
