...
--CREATE UNIQUE NONCLUSTERED INDEX #tmpId ON #p(id)
...
--OPTION(MAXDOP 4)				
--PRINT('Rows are '+CAST(@@ROWCOUNT AS VARCHAR(10)))
...
from (
		...
		from t_FileBack f inner join t_RegisterCaseBack r on
				...
				m.MUCode=t1.MU			
						inner join #p p on
				cb.id=p.id	
		WHERE c.DateEnd>= t1.beginDate AND c.DateEnd<=t1.endDate AND t1.calculationType=1 AND f.DateCreate>= t1.beginDate
		...
		from t_FileBack f inner join t_RegisterCaseBack r on
				...
				m.MUCode=t1.MU	
						inner join #p p on
				cb.id=p.id	
		WHERE c.DateEnd>= t1.beginDate AND c.DateEnd<=t1.endDate AND t1.calculationType=2 AND f.DateCreate>= t1.beginDate
		group by c.rf_idMO,t1.unitCode		
		) t
