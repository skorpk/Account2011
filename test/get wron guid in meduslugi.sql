...
FROM	OPENROWSET(BULK 'c:\Test\HRM175405T34_1405005.XML',SINGLE_BLOB) HRM (ZL_LIST)

SELECT	@patient=LRM.PERS_LIST				
FROM	OPENROWSET(BULK 'c:\Test\LRM175405T34_1405005.xml',SINGLE_BLOB) LRM (PERS_LIST)
...
create table #t6(IDCASE int,ID_C uniqueidentifier,IDSERV nvarchar(36),ID_U VARCHAR(36),LPU nchar(6),PROFIL smallint,
...
SELECT N_ZAP,ID_PAC,IDCASE,ID_C,USL_OK,VIDPOM,
		FOR_POM,VID_HMP,METOD_HMP,--13.01.2014					
...
insert #t6
SELECT IDCASE,ID_C,IDSERV,ID_U,LPU,PROFIL,VID_VME,DET,replace(DATE_IN,'-',''),replace(DATE_OUT,'-',''),DS,CODE_USL,KOL_USL,TARIF,SUMV_USL,PRVS,COMENTU,PODR
...
DECLARE @guidCase UNIQUEIDENTIFIER

SELECT * FROM #t6 WHERE dbo.is_guid(ID_U)=0

--BEGIN TRY
--SELECT ID_C,CAST(id_u AS UNIQUEIDENTIFIER)
--FROM #t6
--print 'OK'
--end try
--begin catch
