use RegisterCases
go
if OBJECT_ID('usp_InsertRegisterCaseDataLPU',N'P') is not null
drop proc usp_InsertRegisterCaseDataLPU
go
create proc usp_InsertRegisterCaseDataLPU
				@doc xml,
			@patient xml,
			@file varbinary(max),
			@countSluch int
...
create table #t5(N_ZAP int,ID_PAC nvarchar(36),IDCASE bigint,ID_C uniqueidentifier,USL_OK tinyint,VIDPOM smallint
				...
				,NPR_MO nchar(6),EXTR tinyint,LPU nchar(6),PROFIL smallint,
					 ...
					 SANK_MEK numeric(15, 2),SANK_MEE numeric(15, 2),SANK_EKMP numeric(15, 2),COMENTSL nvarchar(250),F_SP tinyint,IDDOCT VARCHAR(25))
...
SELECT N_ZAP,ID_PAC,IDCASE,ID_C,USL_OK,VIDPOM,
		...
		PRVS,OS_SLUCH,IDSP,ED_COL,TARIF,SUMV,REFREASON,SANK_MEK,SANK_MEE,SANK_EKMP,COMENTSL,F_SP,IDDOKT
