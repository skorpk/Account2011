...
FROM	OPENROWSET(BULK 'c:\Test\HRT34_M431001120101.xml',SINGLE_BLOB) HRM (ZL_LIST)
...
select c.GUID_Case,c.id,cb.rf_idRecordCaseBack,cb.TypePay,t.pay
--update cb
...
from t_Case c inner join @t t on
		...
		and ab.NumberRegister=2
where cb.TypePay!=t.pay
go
